...
func (r *Router) ProcessAllRoutes() {
	streamPath := "/twitter/stream"
	r.GET(streamPath, func(r *http.Request) (statusCode int, data map[string]interface{}) {
		conf.LOG.Sugar().Info("received GET request at path: %s", streamPath)
		err := ConsumeKafkaStream()
		if err != nil {
			conf.LOG.Sugar().Errorf("error while consuming kafka stream %s", err)
		}
